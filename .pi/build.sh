...
# docker buildx build --tag stevia --file .pi/Dockerfile --platform linux/arm64 --cache-from stevia:cache --cache-to stevia:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar stevia
...
sudo bash -c "echo stevia > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/stevia.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/stevia.img
